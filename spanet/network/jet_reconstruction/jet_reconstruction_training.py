...
class JetReconstructionTraining(JetReconstructionNetwork):
    ...
    def combine_symmetric_losses(self, symmetric_losses: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        symmetric_losses_reduced = torch.log(torch.sqrt(symmetric_losses_reshaped.exp().prod(dim=0)))
        total_symmetric_loss = torch.log(torch.sqrt(symmetric_losses_reduced.exp().prod(dim=1)))
